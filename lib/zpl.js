...
const generateZPL = (data, config) => {
  const arr = generateLabelArray(data, config);
  const zpl = convertTextArrayToZPL(arr, config)
  return zpl;
};

const generateLabelArray = (data, config) => {
  const { maxCharacter } = config;
...
const convertTextArrayToZPL = (data, config) => {
  const { fontSize, xOffset, yOffset } = config;
  ...
  const zpl = arrToZPL(data);
  return zpl;
};

module.exports = { generateZPL, generateLabelArray, convertTextArrayToZPL };
